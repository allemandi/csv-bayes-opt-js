    ...
    for (const c of model.userColumns) {
        ...
        if (model.encoding.text[c]) {
            ...
            for (const cat of cats) {
                const key = `${c}__${cat}`;
                const score = key in encoded ? Number(encoded[key]) : -Infinity;
                if (score > bestVal) {
                    bestVal = score;
                    bestCat = cat;
                }
...
function buildTargetConfigs(encoding, metadata) {
    ...
    for (const c of Object.keys(metadata)) {
        if (encoding.numeric[c]) configs[c] = { type: "number", yKeys: [c], xKeys: allFeatures.filter((f) => f !== c) };
            ...
            const cats = encoding.text[c].categories;
            const catToIdx = cats.reduce((acc, cat, idx) => { acc[cat] = idx; return acc; }, {});
            const yKeys = [c];
            const xKeys = allFeatures.filter((f) => !f.startsWith(`${c}__`));
            configs[c] = { type: "text", yKeys, xKeys, catToIdx };
        }
    }
    return configs;
}
...
function makeDatasetForTarget(encodedRows, rawRows, cfg) {
    const X = encodedRows.map((r) => cfg.xKeys.map((k) => Number(r[k] || 0)));
    const Y = rawRows.map((r) => cfg.yKeys.map((k) => {
        if (cfg.type === "text") {
            return cfg.catToIdx[r[k]];
        } else {
            return Number(r[k] || 0);
        }
    }));
...
function predictTarget(model, target, encodedRow, precomputed = null) {
    ...
    if (cfg.type === "number") {
        ...
        return {
            type: "number",
            mu: model.encoding.numeric[target].min + mu * scale,
            sd: sd * Math.abs(scale || 1),
        };
    } else {
        // text
        let mu, sd;
        ...
        const probs = cats.map((_, idx) => {
            const low = idx - 0.5;
            const high = idx + 0.5;
            return normalCDF(high, mu, sd) - normalCDF(low, mu, sd);
        });
        const sum = probs.reduce((a, b) => a + b, 0) || 1;
        const normalizedProbs = probs.map(p => p / sum);
        let bestIdx = 0;
        for (let i = 1; i < normalizedProbs.length; i += 1) {
            if (normalizedProbs[i] > normalizedProbs[bestIdx]) bestIdx = i;
        }
        const ranked = cats.map((label, idx) => ({ label, probability: normalizedProbs[idx] })).sort((a, b) => b.probability - a.probability);
        return { type: "text", label: cats[bestIdx], probs: normalizedProbs, ranked };
    }
}

// This function trains target-wise Gaussian Process models for all columns.
function trainInternalModel(encodedRows, rawRows, encoding, metadata, lengthScale) {
...
function expectedImprovement(mu, sd, bestSoFar) {
    ...
    const z = delta / sd;
    return Math.max(0, delta * normalCDF(z) + sd * normalPDF(z));
}
